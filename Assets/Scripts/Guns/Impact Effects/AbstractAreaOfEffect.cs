...
namespace LlamAcademy.Guns.ImpactEffects
{
    public abstract class AbstractAreaOfEffect : ICollisionHandler
    {
        ...
        {
            Hits = Physics.OverlapSphereNonAlloc(HitPosition, Radius, HitObjects, Gun.ShootConfig.HitMask);
            for (int i = 0; i < Hits; i++)
            {
                if (HitObjects[i].TryGetComponent(out IDamageable damageable))
                {
                    ...
                    damageable.TakeDamage(Mathf.CeilToInt(BaseDamage * DamageFalloff.Evaluate(distance / Radius)));
                }
            }
        }
    }
